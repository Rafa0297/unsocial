...
describe('deleteComment', () => {
  ...
  it('fails on non-existing user', () =>
    expect(deletePost('012345678901234567890123', '012345678901234567890123')).to.be.rejectedWith(
      NotFoundError,
      /^user not found$/
    ))
  ...
  it('fails on non-existing post', () => {
    ...
    return expect(
      Promise.all([user.save()]).then(([user]) => deletePost(user.id, '012345678901234567890123'))
  ...
  it('fails on not-own post', () => {
    ...
    return expect(
      Promise.all([user.save(), user2.save(), post.save()]).then(([user, user2, post]) => deletePost(user2.id, post.id))
